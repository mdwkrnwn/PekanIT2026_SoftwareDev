...
export function Card({
  ...
  return <section className="mt-14 lg:grid-cols-2 2xl:grid-cols-4 grid gap-8">
    {merchants.map(merchant => <Link href={'/detail'} key={merchant.title} className="border-border overflow-hidden rounded-[2rem] border bg-white transition-all hover:-translate-y-1 hover:shadow-xl cursor-pointer ">
      {
        /* Image */
      }
      <div className="h-80 relative overflow-hidden">
        <Image src={merchant.image} alt={merchant.title} fill className="hover:scale-105 object-cover transition-transform duration-300" />
